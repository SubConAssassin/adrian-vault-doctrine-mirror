...
- 🛑 **NEVER plain-`reboot` M2 — it's HEADLESS (no keyboard/screen) and the disk is FileVault-locked.** A normal reboot lands at the pre-boot unlock screen needing a CONSOLE login M2 doesn't have → SSH/VNC/Tailscale all go dark, un-recoverable remotely until physical access (learned hard 2026-06-20: rebooted to clear swap-thrash → M2 went dark, Adrian not home, no keyboard on it). **To reboot headless M2 safely: `sudo fdesetup authrestart`** (reboots AND unlocks FileVault in one step → lands in a usable session). To clear memory pressure WITHOUT a reboot, SSH in and kill the hogs (idle Antigravity IDEs, surplus `http.server`s, ollama) — never `reboot`. **Prevention TODO when M2 is back: `sudo fdesetup authrestart -delayminutes 0` is the only safe remote reboot; also enable auto-login or disable FileVault for headless 24/7 so a crash-reboot self-recovers.**
