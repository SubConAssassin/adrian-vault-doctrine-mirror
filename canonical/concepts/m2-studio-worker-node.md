...
- **Terminal:** `ssh studio` (alias in `~/.ssh/config` on the MacBook → `subconm2@192.168.1.2`, key `~/.ssh/id_ed25519`). Also reachable on **.53** (dual-homed Wi-Fi+Ethernet). Passwordless.
- **GUI / full desktop:** double-click **`~/Desktop/Mac Studio Screen.inetloc`** (or Finder → Connect to Server → `vnc://192.168.1.2`). Screen Sharing is enabled + authorized for all users.
...
- ⚠️ **CORRECTED 2026-08-11 — FileVault is OFF, confirmed live twice** (2026-07-06/08 per `lessons-learned.md`; reconfirmed 2026-08-11 immediately before a routine restart, `fdesetup status` → "FileVault is Off"). The warning below describes the 2026-06-20 state and is **not the current risk** — the "Prevention TODO" it names (disable FileVault for headless 24/7) was evidently actioned at some point without this doc being updated, so it sat here for weeks telling sessions to fear a risk that no longer existed. **Current safe remote-reboot: plain `sudo shutdown -r now`** — no pre-boot unlock screen to get stuck at, SSH/Screen Sharing come back on their own once macOS finishes booting (both are system-level launchd services, no console login needed). **Still run `fdesetup status` immediately before any remote reboot** — if FileVault is ever re-enabled, the warning below applies again in full and `sudo fdesetup authrestart` becomes the only safe method again.
  - 🛑 *Original 2026-06-20 warning — kept for context, was true then, is NOT the current state:* NEVER plain-`reboot` M2 — it's HEADLESS (no keyboard/screen) and the disk is FileVault-locked. A normal reboot lands at the pre-boot unlock screen needing a CONSOLE login M2 doesn't have → SSH/VNC/Tailscale all go dark, un-recoverable remotely until physical access (learned hard 2026-06-20: rebooted to clear swap-thrash → M2 went dark, Adrian not home, no keyboard on it). To reboot headless M2 *while FileVault is on*: `sudo fdesetup authrestart` (reboots AND unlocks FileVault in one step → lands in a usable session). To clear memory pressure WITHOUT a reboot, SSH in and kill the hogs (idle Antigravity IDEs, surplus `http.server`s, ollama) instead.
...
## 2b. Direct link to the M2 Pro mini — Thunderbolt Bridge (added 2026-08-05)
Adrian ran a physical Thunderbolt cable directly between the Studio and the M2 Pro mini (`ssh m4`). Verified working end-to-end and hardened to **permanent static addressing** (persists reboot — set via `networksetup -setmanual`, not a transient `ifconfig` alias, so it does not need renewing):
- **Studio `10.10.99.1` ↔ Mini `10.10.99.2`**, /24, on the `bridge0` ("Thunderbolt Bridge") service. Friendly hostnames in `/etc/hosts` on **both** machines: `studio-tb` / `mini-tb`.
- **Measured throughput ~1.56GB/s (~12.5Gb/s)** via SSH pipe (`dd | ssh 'cat >/dev/null'`) — 12x+ the gigabit LAN both nodes otherwise share for `ssh studio`/`ssh m4` traffic. Physical link negotiates 40Gb/s (`system_profiler SPThunderboltDataType`, Domain UUIDs cross-match both ends, ruling out a stray/other device).
- SSH host keys for the new addresses pre-accepted both directions — `ssh subconm4@mini-tb` from the Studio and `ssh subconm2@studio-tb` from the Mini both work with no first-use prompt.
- **Not yet wired into any pipeline/transfer tooling.** It's a ready, permanent, named fast path for Studio↔Mini data movement that bypasses the LAN/switch entirely — reach for it (instead of `scp`/`rsync` over `ssh studio`/`ssh m4`) whenever a job moves large data directly between these two specific nodes (corpora, model weights, Blender renders, etc).
...
- **Endpoint test:** `curl -s http://192.168.1.2:11434/api/tags`. Only **ONE** `ollama serve` should run — if you ever see `bind: address already in use`, kill all (`pkill -9 -f ollama`) and `launchctl load ~/Library/LaunchAgents/com.adrianvault.ollama.plist`.
