...
> **⚠️ SUPERSEDED/CORRECTED 2026-05-21 (post-migration first-boot audit on the new 64GB MacBook).** Google has **rebranded the desktop app from "Antigravity" to "Antigravity IDE"** at some point between 2026-05-19 and 2026-05-21 (version 2.0.1 of the new bundle was installed). The bundle id changed: **OLD `com.google.antigravity` → NEW `com.google.antigravity-ide`**; app name **OLD `Antigravity.app` → NEW `Antigravity IDE.app`**; process name **OLD `"Antigravity"` → NEW `"Antigravity IDE"`**. The empty `/Applications/Antigravity.app/` directory remains as a leftover on disk; the real app is at `/Applications/Antigravity IDE.app`. macOS LaunchServices keeps the old bundle id phantom-registered (osascript `tell application id "com.google.antigravity"` still returns "Antigravity" — a phantom — even though the app is gone), so the 2026-05-17 frontmost guard above silently auto-yielded **every** cycle: the real frontmost reports `com.google.antigravity-ide`, the guard compared against `com.google.antigravity`, → always-not-matched → never keystroked. **Same shape as the Electron trap — different trigger (rebrand vs Electron name).** **Authoritative 2026-05-21 fix:** gate on **`bundle identifier == com.google.antigravity-ide`**; pgrep against `/Applications/Antigravity IDE.app`; AppleScript `tell process "Antigravity IDE"`; activate via `tell application id "com.google.antigravity-ide"`; `open -a "Antigravity IDE"`. Patched across 8 tool files (35 sites): `tools/{bounce-to-ag,ui-autoloader,hive-supervisor,burn-watchdog,burn-status,burn-preflight,ag-ui-recovery,ag-prefs}*`. Reversibility snapshot: `working/_snapshots/ag-rename-patch-20260521-092130/`. The *frontmost-or-abort principle still stands*; the *bundle-id-not-process-name principle still stands*; only the specific bundle-id string and app name need to stay current with Google's product naming. If Google renames again, repeat this drill — the SUPERSEDED block pattern is the durable mechanism.
