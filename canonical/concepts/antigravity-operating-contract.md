...
> **⚠️ CORRECTED 2026-08-04 (Adrian-direct, 2026-07-29; memory: `gemini-subscription-downgraded-from-ultra`; full correction: `canonical/concepts/delegation-first-operating-doctrine.md` §15).** The figures below described the **Gemini Ultra** subscription. Adrian downgraded it to a ~$20-30 basic tier on 2026-07-29 — his own words: *"predominantly because you failed to use it when I had the Ultra account."* **The 30,000,000+ tokens/day target and "practically unlimited" framing are no longer true and must not be treated as a live burn ceiling.** Do not ration into artificial paranoia either — but the correct posture now is: work the task fully and grind on genuinely bounded, well-specified jobs, without assuming a Ultra-scale pool sits behind you. Direct quota exhaustion is an explicit `RESOURCE_EXHAUSTED (code 429): Individual quota reached ... Resets in ...` response; current print mode exits non-zero. `THIN rc=0` was a local wrapper artifact corrected 2026-09-04; an empty success is ambiguous, not an auth or quota diagnosis.

**(Historical, Ultra-era — kept for context, NOT the current rule):** *"You have practically unlimited tokens (over 1 million tokens per hour). The daily target is 30,000,000+ tokens per day."*
...
- **2026-09-04 (Factual correction — quota failure surface):** Google’s current headless contract and CLI changelog confirm that server-side failures go to stderr and return non-zero. The prior `THIN rc=0 = quota exhaustion` wording described a local `agy-ask.py` status-propagation bug, not the current direct CLI contract. Wrapper repaired; empty success is now `AMBIGUOUS_EMPTY`. No anti-confabulation, spend, or safety rule changed.
- **2026-08-04 (Correction — §2 RULE OF UNLIMITED BURN):** Adrian-direct, live in-conversation (*"Make sure everything is fully handled and saved into the prompt architecture to make it reliable moving forward."*), following his 2026-07-29 subscription-downgrade decision (memory: `gemini-subscription-downgraded-from-ultra`). §2's "practically unlimited... 30,000,000+ tokens/day" described the Gemini **Ultra** plan, cut to a ~$20-30 basic tier on 2026-07-29 ("predominantly because you failed to use it when I had the Ultra account"). Corrected in place with a dated note; original text kept, labelled historical/Ultra-era, not deleted. No other section of this contract is affected — the anti-confabulation rules (§1, §3-§9) are unamended and unweakened. Full correction: `canonical/concepts/delegation-first-operating-doctrine.md` §15.
