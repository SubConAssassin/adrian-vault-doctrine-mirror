...
capacity_update: "2026-08-04 — ⚠️ CORRECTED (Adrian-direct, 2026-07-29; memory: gemini-subscription-downgraded-from-ultra): the Google/Antigravity subscription referenced throughout this document (Layer 2, 'agy = biggest/most-tokens pool', the 30M+ tokens/day target and every 'burn rate per dollar' re-framing of it) was the Gemini ULTRA plan and has since been downgraded to a ~$20-30 basic tier. agy is now a small, scarce pool — do not plan capacity against any 30M+/day figure in this file. This document's node topology (M3 worker) is also superseded by the current fleet (M1 Max + M2 Mac Studio + i7 Intel Mac; see multi-machine-infrastructure memory). Current routing law lives at canonical/concepts/delegation-first-operating-doctrine.md §15 — treat this file as historical/architectural reference only, not a live capacity target."
