...
cost-limit: Reserve strictly for what ONLY Claude can do. Never let Claude run dry before agy/grok/codex/M3/M1 are saturated. Per-chat and rolling window both enforced.
...
must-not-do: Pure one-shot deep research better suited to grok/codex web bridges; high-volume low-reasoning grunt (that goes to M3 local).  
...
**M3 (local Mac node, 18GB RAM, 24/7, no API limits)**  
should-do: HIGH-VOLUME LOW-REASONING grunt ONLY on big corpuses — bulk classification, embeddings, Whisper transcription, first-pass corpus extraction (entities, themes, facts, timelines from raw/notes/, raw/chatgpt-import/, external-research-in/), deduplication, simple tagging, moondream vision bulk image description. This is the primary offload for first-pass work.  
...
cost-limit: $0 / unlimited tokens. Constrained only by 18GB RAM — size batches accordingly. Must stay 100% loaded 24/7; never idle.
...
5. Is it high-volume low-reasoning grunt on large raw corpuses (bulk classification, first-pass extraction, transcription, embeddings, dedup, bulk vision)? → M3 local LLM (ollama/Whisper/moondream) via `tools/m3.sh` or ssh. This is mandatory offload — never send big-corpus first-pass to primary Mac cloud CLIs (grok/codex/agy).

6. Is it local GPU work on primary and machine is idle with headroom under the 2-4 CLI cap? → M1 Max local tools. Secondary to M3.
...
- GREEN: Full push on M3/agy/grok/codex. Claude ceiling: high-value only, max ~15-20% of remaining weekly budget for decisions/verification/operator comms. Run team at 90-95% of empirically found throttle.
...
- RED: Claude does zero new work. Only complete verification of in-flight and emergency operator directives. Force deferral. Immediate max load on M3 + feeder. Escalate to operator if projection shows Claude dry before resources exhausted.
...
General loop (delegated): baseline current throughput (tasks/h, tokens/day, quality via 10% spot verification), increment load 10-20%, run sustained window (hours for CLI, overnight for feeder/M3), measure quality/error/machine health/actual spend, back off on first degradation, lock new baseline, log to state/.
...
- M3: ramp parallel ollama + Whisper + extraction batches. Monitor 18GB RAM/CPU pressure, completion rate, first-pass acceptance on spot checks. Target 24/7 85%+ utilization. Secretary must always preload next 8-12h of work so node never idles.
...
**What the plan contains:** Prioritized per-node task list with ready-to-paste prescriptive prompts; explicit objectives, inputs, output specs, verification method (e.g. handoff or extracted/ location); ROI justification tied to active ledgers; list of all deferred items from prior triage with fresh-chat instructions; concurrency schedule for primary (max 2-4 slots); M3 batch scripts prepped; feeder queue ready.

**How M3 + M1 Max + AG-feeder get loaded nightly (secretary executes, Claude verifies only if GREEN):**
- M3 (primary volume offload): secretary prepares 8-12h continuous batch via tools/m3.sh enqueue or ssh scripts. Covers new audio for Whisper, new raw/notes/ + external-research-in/ for first-pass extraction/classification/dedup/embeddings, image dirs for moondream. M3 (always-on) runs autonomously. Outputs land in working/extracted/ or tentative canonical for morning review.
- M1 Max: only headroom local jobs (secondary embeddings, vision) if primary concurrency and idle allow. M3 takes the heavy volume.
...
Routing: correctness/orchestration/verify/operator → you. Deep research (one-shot, citations) → grok or codex; prefer their web Deep Research bridges (0 primary CLI slots). Long-horizon agentic/execution/coding/overnight → agy via `bash tools/cli-ask.sh agy "..."` or `tools/ag-feeder.py --until HH:MM` (1 slot). High-volume low-reasoning grunt on big corpuses (bulk classif/extract/transcribe/embed/dedup/vision) → M3 local LLM (ollama/Whisper/moondream) via tools/m3.sh or ssh. Never route big-corpus first-pass to primary cloud CLIs.

Hard cap primary Mac concurrent cli-ask at 2-4 (shared with IDE; AG-feeder counts 1). Offload to M3 + web bridges. 24/7: by 22:00 delegated secretary produces + loads overnight plan into M3 queue, M1 local (headroom only), AG-feeder. Team works while you sleep. Wake: review outputs first. Department ROI mandatory on every allocation (see NODE-BUDGET TABLE). Push all nodes to throttle empirically, then run just under it. M3 must never idle.
```
...
| M3 (local) | $0; 18GB RAM; 24/7. | High-volume first-pass processing of raw corpuses into structured data (transcripts, extractions, embeddings, classifications, deduped sets, vision descriptions). | Volume processed (notes/audio-hours/GB per day); first-pass acceptance rate on downstream spot checks; enables 10x+ more corpus throughput than cloud-only. Must stay loaded. |
