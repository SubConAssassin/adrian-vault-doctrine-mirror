...
All $0 unless marked, all proven. Fire via `bash tools/cli-ask.sh <lane> "PROMPT"`. **SEVEN dispatch families:** **1** `claude` (M2 Studio-only coordinated Claude Code route; Original Siberian Blue identity checked before inference; no Mini/UK Photon or primary-Mac fallback) — **2** `codex` · `codex-sol|codex-terra|codex-luna` — **3** `grok` · `grok-web` (live web, citations mandatory; the `composer` model ID is dead and its replacement is `codex-terra --effort low`) — **4** `agy` — **5** `qwen` · `qwen-plus` (flat-rate Model Studio Token Plan via the live `bl` integration) — **6** `deepseek` (⚠️ **METERED** — `metered-guard.py` and the $20/month trial cap bind) — **7** `local` · `local-fast` · `local-vision` (**$0, unmetered, unlimited**). Flags: `--model M` · `--effort low…ultra`. **Concurrency caps:** codex unbounded · grok 2 · agy 2 · qwen 2 · deepseek 1 · local 3. **THE SATURATION LAW:** a flat-rate lane left idle is throughput destroyed, not saved. Bulk classification/extraction/tagging over >~200 items belongs on `local` by default. **Engine specs, prices and benchmarks live in [[llm-capability-map-2026-09-04]] — this section is invocation + routing only.**
...
Superseded by §14 where they conflict. The current engine board lives in [[llm-capability-map-2026-09-04]]; the still-live content is folded forward into **§4 and §6a** (lane invocation, `grok-web` for post-cutoff facts, the file-read delivery idiom, and Grok `/goal` overnight builds), **§8** (session logs as a truth channel), and **§4** (corporate notes). Retired routing deltas — Terra as the everyday default, Composer for mechanical edits, and >400K payload re-routing — are not folded forward. Full original text: the archive.

## §14 — THE CURRENT MODEL LAW (Claude-5 / July-25; added 2026-07-25)

**§14.1 — The board.** Engine specs, prices and benchmarks: **[[llm-capability-map-2026-09-04]]** — that file is the detail; this section is the binding law. **REFRESHED 2026-09-04:** **Opus 5 remains the default and has had NO revision since 24 Jul** — a confirmed negative checked across five first-party Anthropic pages; the post-July bump is a different model, Fable 5.1. **Sonnet 5 is now $2/$10 PERMANENTLY** (the scheduled 1 Sep rise to $3/$15 did not happen). **Fable 5.1** (1 Sep) is the strongest model on the board but is **not** promoted, and the reason is compatibility rather than the contested cost figure: it is a **Covered Model requiring 30-day retention (400s under ZDR)**, it **400s on forced `tool_choice`**, it has no Priority Tier and no fast mode, it needs Claude Code v2.1.251+, and **on Max it is capped at 50% of the weekly usage limit**. Its cache reads are $0.25/MTok, genuinely half Opus 5's $0.50 in absolute dollars. **The widely-repeated "1.7x output tokens / +20% cost per task" claim was NOT FOUND in first-party material by three independent legs — do not encode it as a routing constant.**
...
**§14.5 — Tier-pinning, current assignments (§1b.3's law, restated against the current board).** The allowlist is unchanged; only the engines are renamed. **Architect/final-verify → Opus 5** (**Fable 5.1** only when evals on Opus 5 at `xhigh`/`max` demonstrably fall short — a capability ceiling, not a stopwatch; Anthropic itself assigns multi-hour autonomous coding agents to Opus 5. Check the 50% Max cap before committing a long Fable run). **Builder → Sonnet 5 @ medium** (beats last generation's top tier on agentic coding, and at **$2/$10 permanently** — the intro price became the standard price — it is the single biggest cost win inside Claude). **Scribe → Haiku 4.5** (no effort parameter, 200k ctx). **Auditor → a different model family from the builder.**

**§14.6 — Two engine-specific hard rules.**
- **Grok hallucination: RE-MEASURED 2026-09-04.** The binding 54% is a **Grok 4.5** figure. **Grok 4.6 measures 34.3% hallucination / 48.2% accuracy / Omniscience Index 30.5** (AA-Omniscience 26-27 Aug snapshot). ⚠️ **Sourcing caveat that must travel with the number:** both legs reached it through an aggregator, the direct AA-Omniscience row for 4.6 was **[NOT FOUND]**, and xAI's own model card points the OTHER way on an internal factuality eval. **So: update the number, keep the rule.** §8's different-family cross-check before promotion remains a **numeric requirement**.
- **GPT-5.6 Luna: the "41.3" is LONG-CONTEXT RECALL, not reasoning.** Corrected 2026-09-04. The figures (Sol 91.5 / Terra 89.6 / **Luna 41.3**) are **MRCR v2 8-needle retrieval**, from OpenAI's own GPT-5.6 release — not a general reasoning score, and not "Nerova". The operating rule is unchanged but its scope is narrower and better aimed: **Luna is fine for schema-bound classification and extraction over SHORT documents, and must never be given needle-in-a-haystack work over long context.**

**§14.7 — The agy pin.** `tools/agy-ask.py` MUST pin the **exact lowercase slug** published by `agy models` — a wrong-format pin matches nothing and silently falls back to the last-selected model, the exact failure the pin exists to prevent (found 2026-07-25: the old `"Gemini 3.5 Flash (High)"` string was both superseded and non-binding). Pin history: → **`gemini-3.6-flash-high`** (2026-07-25) → **`gemini-3.7-flash-high`** (2026-08-14) → **`gemini-3.8-flash-high`** (2026-09-04, Gemini 3.8 Flash GA 2 Sep; verified live end-to-end, rc=0). ⚠️ **The silent-fallback hazard this section is built on NO LONGER APPLIES to the headless path**: Google's current headless docs say an unknown `--model` **exits non-zero with an ERROR listing available models**. Sticky last-selected behaviour is an *interactive UI* property. The pin still matters — it stops the lane drifting a generation behind the surface it rides on, which is what actually happened twice — but a wrong slug now fails loudly rather than quietly.
...
**Resolved 2026-09-04:** the fixed 6:3:1 split was retired. Route by capability and measured live capacity instead: Codex Pro 5x is the biggest subscription pool, agy remains quota-fragile, and local remains unmetered. The current measurements and sibling-file disposition live in [[llm-capability-map-2026-09-04]] and §4 above.
