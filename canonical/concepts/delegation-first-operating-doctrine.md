...
- **`bash tools/cli-ask.sh agy "PROMPT"`** — **Gemini 3.6 Flash High** (repinned 2026-07-25, §14.7; was 3.5 Flash High). **Default grind engine** (biggest quota pool). 1M ctx / 65K out, live Google-Search grounding in-CLI, best tool-use reliability in the stack (MCP Atlas 83.6%), the $0 multimodal/vision lane. Reads serials/images reliably. Process-group SIGKILL on timeout. Old `gemini` CLI retired by Google 2026-06-18 — agy is the only lane. ⚠️ **Concurrency gate: max 2 agy clients** (`CLI_ASK_AGY_MAX`) — it wedges all lanes above that.
  - ⚠️ **KNOWN DEFECT — doctrine and code disagree on large payloads.** `tools/cli-ask.sh:123` still auto-routes any prompt **>100,000 chars to grok**. That was written when grok had a 1M window; since Grok 4.5 grok has the **smallest** window of the three (500K) *and* a 54% hallucination rate, so the script sends the biggest jobs to the least suitable engine — the opposite of §13.3.6. **Not breaking today** (100k chars ≈ 25k tokens, well inside 500K, and the >40KB file-read idiom handles delivery) but it bites at the top end and taints auto-routed jobs with grok's hallucination profile. **Doctrine is §13.3.6: route big payloads to agy/codex (1M-class), not grok.** Fix proposed but NOT applied — it changes live routing behaviour, so it needs Adrian. Tracked: `canonical/system/prompt-map.md` §5.
...
**§14.8 — OPEN, needs Adrian (doctrine change under AGENTS.md §8):** our own constitutional files (`AGENTS.md`, `CLAUDE.md`, this file, `claude-ceo-operating-doctrine.md`) repeat the same rules many times over and load into **every session**. On 2025-era models that redundancy was insurance; on Claude 5 the vendors' own measurements say it is costing quality and burning tokens. A consolidation pass is the highest-value unrealised saving in the stack. **Not actioned unilaterally.**
