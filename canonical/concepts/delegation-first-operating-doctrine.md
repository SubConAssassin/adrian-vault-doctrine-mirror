...
All $0 unless marked, all proven. Fire via `bash tools/cli-ask.sh <lane> "PROMPT"`. **EIGHT dispatch families:** **1** `claude` (M2 Studio-only coordinated Claude Code route; Original Siberian Blue identity checked before inference; no Mini/UK Photon or primary-Mac fallback) — **2** `codex` · `codex-sol|codex-terra|codex-luna` — **3** `grok` · `grok-web` (live web, citations mandatory; the `composer` model ID is dead and its replacement is `codex-terra --effort low`) — **4** `agy` — **5** `qwen` · `qwen-plus` (flat-rate Model Studio Token Plan via the live `bl` integration) — **6** `deepseek` (⚠️ **METERED** — `metered-guard.py` and the $20/month trial cap bind) — **7** `local` · `local-fast` · `local-vision` (**$0, unmetered, unlimited**); **8** `cloud` (asynchronous Claude Code cloud sessions on a one-time $250 credit, dispatched from M1; bullet below). Flags: `--model M` · `--effort low…ultra`. **Concurrency caps:** codex unbounded · grok 2 · agy 2 · qwen 2 · deepseek 1 · local 3. **THE SATURATION LAW:** a flat-rate lane left idle is throughput destroyed, not saved. Bulk classification/extraction/tagging over >~200 items belongs on `local` by default. **Current model facts and prompt mechanics live in [[token-enriched-delegation-prompting-protocol-2026-09-23]] and [[llm-capability-map-2026-09-23]] — this section is invocation + constitutional routing only.**
