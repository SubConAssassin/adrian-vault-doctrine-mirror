...
- **`bash tools/cli-ask.sh codex "PROMPT"`** — GPT-5.5. Deep/structured. ✅ FLAT-RATE on Adrian's ChatGPT Plus/Pro subscription — NOT metered (corrected 2026-06-16 Adrian-direct; the metered surface is the `ask-chatgpt.py` API, NEVER the CLI) → hit it to its throttle ceiling like grok/agy. "Slow crawler" note predates GPT-5.5 (Apr 23, fewer tokens/task) — re-test before treating as a constraint. New: `/goal` mode (self-chains to a verifiable end-state), TOML subagents, `codex exec resume --output-schema` for resumable overnight runs.
...
**§10.3 — The multi-pod fleet** (live as of 2026-06-18: M1 Max MacBook + M2 Mac Studio; M3 SOLD, M1 Ultra purchase cancelled):
