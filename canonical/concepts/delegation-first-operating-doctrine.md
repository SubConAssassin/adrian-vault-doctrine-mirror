...
- **16.3 Account identity and host — updated Adrian-direct 2026-09-13.** Coordinated Claude CLI work uses `bash tools/cli-ask.sh claude "PROMPT"`: M2 Studio (`studio`) only, verifying `originalsiberianblue@gmail.com` with the target CLI before inference. Fail closed if unavailable, unauthenticated, at capacity or the identity differs; never fall back to Mini/UK Photon or the primary Mac. Adrian explicitly authorises this existing OSB account and reserves UK Photon Utilities for his own direct use; his stated 95% usage is a user report, not telemetry. This replaces the former Mini/UK Photon default and OSB exclusion for this route. Preserve existing sessions, fleet priorities, permissions, spending gates and user-only resets. A direct `claude` command is local and is not this coordinated route. General host guidance in §1/§5.1 does not override this Claude-specific account restriction.
