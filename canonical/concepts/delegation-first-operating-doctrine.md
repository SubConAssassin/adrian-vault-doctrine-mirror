...
All $0 unless marked, all proven. Fire via `bash tools/cli-ask.sh <lane> "PROMPT"`. **SIX dispatch families:** **1** `codex` · `codex-sol|codex-terra|codex-luna` — **2** `grok` · `grok-web` (live web, citations mandatory; the `composer` model ID is dead and its replacement is `codex-terra --effort low`) — **3** `agy` — **4** `qwen` · `qwen-plus` (flat-rate Model Studio Token Plan via the live `bl` integration) — **5** `deepseek` (⚠️ **METERED** — `metered-guard.py` and the $20/month trial cap bind) — **6** `local` · `local-fast` · `local-vision` (**$0, unmetered, unlimited**). Flags: `--model M` · `--effort low…ultra`. **Concurrency caps:** codex unbounded · grok 2 · agy 2 · qwen 2 · deepseek 1 · local 3. **THE SATURATION LAW:** a flat-rate lane left idle is throughput destroyed, not saved. Bulk classification/extraction/tagging over >~200 items belongs on `local` by default. **Engine specs, prices and benchmarks live in [[llm-capability-map-2026-09-04]] — this section is invocation + routing only.**
...
- **16.3 Account identity.** Verify which account a session is on before relying on it. Do not unlock the old protected UK Photon profile on a guess. The protected OSB account is untouched by this policy.
