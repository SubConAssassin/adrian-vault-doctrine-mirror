...
| **Builder / Execution engine** | Antigravity (⚠️ **Gemini base tier, not Ultra — downgraded ~2026-07-29, see note below**) | Sustained high-throughput build work, file IO, code, scraping, long synthesis, Imagen for image generation | ⚠️ **No longer "daily envelope much larger than Claude's" — the Ultra plan this row described was cut to a ~$20-30 basic tier.** Treat as a scarce/opportunistic lane, not the default largest-pool engine. |
...
> **⚠️ CORRECTED 2026-08-04 (Adrian-direct, 2026-07-29; memory: `gemini-subscription-downgraded-from-ultra`).** The Builder row above described the **Gemini Ultra** subscription ($200, 20×). Adrian downgraded it to a ~$20-30 basic tier on 2026-07-29 — his own words: *"predominantly because you failed to use it when I had the Ultra account."* AG/agy is **no longer the largest-pool, empirically-un-throttled engine** the row above describes; it is now a small, scarce lane, comparable to or smaller than the other two flat-rate CLIs. Full correction and downstream operational consequence: `canonical/concepts/delegation-first-operating-doctrine.md` §15. This does not change AG's *kind* of strength (build/file-IO/scraping/synthesis) — only its relative *capacity* ranking.
...
> **⚠️ CORRECTED 2026-08-04 — this §4.1 anti-pattern predates the 2026-07-29 Ultra→base downgrade and its "AG has no such constraint" premise no longer holds unqualified.** AG's pool is now small and scarce (memory: `gemini-subscription-downgraded-from-ultra`; full correction: delegation-doctrine §15). "Sustained max-burn" still describes the *right posture toward AG's own weekly-lockout ceiling* (below), but do not read this section as licence to assume AG has the deepest pool in the team — it currently does not.
...
2. **Auto-NUDGE (PARTIAL — read the correction)** — Watchdog ALSO calls `tools/ag_nudge.py file --tier 1` which writes a §10.4-compliant claude-to-ag NUDGE handoff. ⚠️ **CORRECTION 2026-05-29 (audit `wf_a691ad22`):** the NUDGE does **NOT** auto-trigger AG. `com.adrianvault.autoloader` runs `autoloader_daemon.py`, which was **demoted to notify-only on 2026-05-16** (it only raises a macOS notification — it never keystrokes AG). The real keystroker, `ui-autoloader.sh`, is **not a LaunchAgent** and is **off by default**. So a NUDGE lands as a file with **nothing listening to type it into AG unless a feeder (`ag-feeder.py`) or `ui-autoloader.sh` is already running**. Treat the nudge ladder (steps 2–4) as producing escalation *files*, not as self-healing AG. True self-healing needs the AG CLI/SDK driver, which is **unbuilt** — see `canonical/concepts/cross-model-automation-protocol.md §8`. **Operating rule until then: a running feeder is the prerequisite for any AG auto-recovery; if no feeder is up, these nudges do nothing.**
